...
pattern_defense_gauge = Gauge('ml_pattern_defense_rate', 'Pattern defense success rate', ['pattern'])
learning_stability_gauge = Gauge('ml_learning_stability', 'Learning stability score')
forgetting_events_counter = Counter('ml_catastrophic_forgetting_total', 'Total catastrophic forgetting events')
...
class LearningStabilityMonitor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.pattern_performance: Dict[str, deque[float]] = defaultdict(lambda: deque(maxlen=100))
    ...
    async def check_stability(self, current_snapshot: PerformanceSnapshot) -> StabilityReport:
        ...
        is_stable = (
            ...
            len([r for r in risk_factors if 'catastrophic' in r.lower()]) == 0
        )
    ...
    def _calculate_stability_score(self, current: PerformanceSnapshot) -> float:
        ...
        perf_drop = self.baseline_performance.overall_accuracy - current.overall_accuracy
        perf_score = max(0, 1 - (perf_drop / self.catastrophic_threshold))
        scores.append(perf_score)
        
        # Consistency score
        if len(self.performance_history) >= 10:
            recent_performances = [s.overall_accuracy for s in list(self.performance_history)[-10:]]
            consistency = 1 - (np.std(recent_performances) / np.mean(recent_performances))
        ...
        for pattern, current_score in current.pattern_defense_rates.items():
            ...
            pattern_stability = min(1.0, current_score / baseline_score) if baseline_score > 0 else 1.0
    ...
    def _identify_risk_factors(self, current: PerformanceSnapshot) -> List[str]:
        ...
        if perf_drop > self.catastrophic_threshold:
            risks.append(f"Catastrophic forgetting detected: {perf_drop:.2%} performance drop")
        ...
        for pattern, current_score in current.pattern_defense_rates.items():
            ...
            if baseline > 0 and current_score < baseline * 0.8:
                risks.append(f"{pattern} defense degraded by {(1 - current_score/baseline):.2%}")
            ...
            if pattern in self.pattern_performance:
                ...
                if recent_scores:
                    analysis[pattern] = {
                        ...
                        'stability': 1 - np.std(recent_scores) if len(recent_scores) > 1 else 1
                    }
    ...
    def _generate_recommendations(self, current: PerformanceSnapshot, 
                                 risks: List[str], 
        ...
        if any('catastrophic' in r.lower() for r in risks):
            recommendations.append("Reduce learning rate to prevent catastrophic forgetting")
        ...
        for pattern, analysis in pattern_analysis.items():
            if analysis['trend'] == 'degrading':
                ...
                recommendations.append(f"Increase {pattern} pattern examples in training")
    ...
    async def _check_alerts(self, report: StabilityReport, snapshot: PerformanceSnapshot):
    ...
    def plot_performance_history(self, save_path: Optional[str] = None):
        ...
        axes[0, 0].axhline(y=self.baseline_performance.overall_accuracy, 
    ...
    def get_summary_stats(self) -> Dict[str, Any]:
        ...
        recent_performances = [s.overall_accuracy for s in list(self.performance_history)[-20:]]
        
        return {
            'current_performance': self.performance_history[-1].overall_accuracy,
            'baseline_performance': self.baseline_performance.overall_accuracy if self.baseline_performance else None,
            'average_performance': np.mean(recent_performances),
            'performance_std': np.std(recent_performances),
            'stability_score': self.stability_scores[-1] if self.stability_scores else None,
            ...
            'alerts_triggered': sum(1 for e in self.forgetting_events if e.get('alert_triggered'))
        }
...
async def monitor_learning_stability(model, pipeline, config: Dict[str, Any]):
    ...
    while True:
        try:
            ...
            snapshot = await monitor.evaluate_model(
                model, 
            ...
            # Log report
            logger.info(f"Stability Report: {report.is_stable} "
