...
class CoordinationLearningBridge:
    ...
    async def connect_to_services(self):
        ...
        try:
            ...
            # Send handshake
            await self.cl_websocket.send(json.dumps({
                'type': 'handshake',
        ...
        if progress.get('lossesAnalyzed', 0) % 10 == 0:
            message = {
                ...
                'payload': {
                    ...
                    'buffer_utilization': progress.get('bufferSize', 0) / 100000  # Normalized
                }
            }
    ...
    async def heartbeat_loop(self):
        ...
        while self.running:
            try:
                ...
                if self.coord_websocket:
                    await self.coord_websocket.send(json.dumps({
                        'type': 'heartbeat',
                        'service': 'learning_bridge'
                    }))
    ...
    async def reconnect_coordination(self):
        ...
        for attempt in range(5):
            try:
                ...
                logger.error(f"Coordination reconnection attempt {attempt + 1} failed: {e}")
