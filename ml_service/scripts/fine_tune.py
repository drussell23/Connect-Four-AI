...
def fine_tune(
    ...
    logger.info(
        f"Mixed dataset: {len(historic_ex)} historic + {len(live_ex)} live examples"
    )

    # Skip fine-tuning if no examples
    if len(mixed_ex) == 0:
        logger.info("No training examples (historic+live) – skipping fine-tuning")
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--base-model", "-b", required=True, help="Path to base checkpoint"
    )
    parser.add_argument(
        "--buffer", "-B", required=True, help="Path to replay_buffer.pkl"
    )
    parser.add_argument(
        "--output", "-o", required=True, help="Path to write new checkpoint"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.8,
        help="Ratio of historic vs new data",
    )
