...
# fallback if training produced nothing
if [[ ! -s "$OUT" ]]; then
  cp "$ROOT/models/current_policy_net.pt" "$OUT"
fi

# Stage 3: evaluate
log "Stage 3: Evaluating new model"
...
# Stage 4: promote
log "Stage 4: Promoting model if threshold met"
